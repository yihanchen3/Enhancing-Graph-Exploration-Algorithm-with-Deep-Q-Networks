...
class AgentNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, batch: torch.Tensor, edge_feat: Optional[torch.Tensor] = None, fgs: Optional[torch.Tensor] = None, name: Optional[str] = None):
        batch_size = (batch.max() + 1).item()
        num_nodes_batch = x.size(0)
        num_agents_batch = int(self.num_agents*batch_size)

        # print('fgs', fgs.size(), fgs)
        # print('name', name)
        ...
        for i in range(self.num_steps+1):
            ...
            if i > 0: 
                ...
                # print('visited_nodes', visited_nodes.size(), visited_nodes)
                # print('num_agents_batch', num_agents_batch)
                ...
                for idx,agent in enumerate(agent_pos):
                    if fgs[agent] == 1:
                        reward[idx] = 1.0
                    else:  
                        reward[idx] = -1.0
