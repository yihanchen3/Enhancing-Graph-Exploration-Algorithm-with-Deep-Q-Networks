...
class AgentNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, batch: torch.Tensor, edge_feat: Optional[torch.Tensor] = None):
        ...
        print('adj', adj, 'size of adj', adj.sparse_sizes(), 'num edges', adj.nnz())
        ...
        for i in range(self.num_steps+1):
            ...
            print('size of agent_pos',agent_pos.size())
            print('agent_node', agent_node)
            print(self.importance_init)
            ...
            if i > 0: 
                ...
                if self.random_agent:
                    ...
                    if self.basic_global_agent or self.basic_agent or self.bias_attention:
                        ...
                        print('neighbors_visited', neighbors_visited)
                        print('size of neighbors_visited', neighbors_visited.size(), 'mask_old', mask_old.size(), 'previous_visited', previous_visited.size())
                    ...
                    if self.basic_agent and not self.bias_attention:
                        ...
                        print('number of nodes', num_nodes_batch, 'number of agents', num_agents_batch)
                        print('node_emb', node_emb.size(), 'agent_node', agent_node.size(), 'agent_neighbour', agent_neighbour.size(), 'agent_emb', agent_emb.size())
                        print(node_emb[agent_node[1]].size(), node_emb[agent_node[1]][agent_neighbour[0]].size(), agent_emb.size())
                        print('agent_emb size duplicated',agent_emb[agent_neighbour[0]].size())
                        ...
                        if edge_feat is not None:
                            ...
                            print(node_emb[agent_neighbour[1]].size(), node_emb[agent_node[1]][agent_neighbour[0]].size())
                            K = torch.cat([node_emb[agent_neighbour[1]], node_emb[agent_node[1]][agent_neighbour[0]]], dim=-1)   
                            
                        print('Q', Q.size(), 'K', K.size())
                    ...
                    print('agent_neighbour_attention_value', agent_neighbour_attention_value.size(), 'attn_score', attn_score.size())

                    print('agent_neighbour_attention_value', agent_neighbour_attention_value[:10], 'attn_score', attn_score[:10], 'agent_neighbour', agent_neighbour[0][:10])
                    
                    print('attn_score', attn_score[:30], 'agent_neighbour', agent_neighbour[0][:30])
                    q = scatter_max(attn_score, agent_neighbour[0], dim=0, dim_size=num_agents_batch)[0]
                    print('q', q.size(), 'q', q[:10])
                    del attn_score
                ...
                print('reward', reward.size(), 'mask_old_agent',mask_old_agent.size(), 'agent_pos', agent_pos.size())
                print('reward', reward[:10], 'mask_old_agent', mask_old_agent[:10],'agent_pos',agent_pos[:10])
