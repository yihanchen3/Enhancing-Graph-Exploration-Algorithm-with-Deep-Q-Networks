...
from model_1_b import AgentNet, add_model_args
...
def train(model, device, loader, optimizer, task_type, use_aux_loss=False, verbose=True):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration", disable=not verbose)):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            if use_aux_loss:
                pred, aux_pred, dqn = model(batch.x, batch.edge_index, batch.batch, batch.edge_attr)
            else:
                pred, _, dqn = model(batch.x, batch.edge_index, batch.batch, batch.edge_attr)
...
def eval(model, device, loader, evaluator, use_aux_loss=False):
    ...
    rewards = rewards.mean().numpy()

    input_dict = {"y_true": y_true, "y_pred": y_pred}

    return evaluator.eval(input_dict), rewards
...
def main(args, cluster=None):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print({'Train': [train_perf, train_reward] , 'Validation': [valid_perf,valid_reward], 'Test': [test_perf,test_reward], 'LR': lr, 'Mem': round(torch.cuda.max_memory_allocated()/1024.0**3, 3), 'Cached': round(torch.cuda.max_memory_reserved()/1024.0**3, 3)})
