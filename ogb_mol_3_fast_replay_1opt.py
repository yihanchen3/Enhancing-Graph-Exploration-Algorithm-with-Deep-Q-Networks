...
def train(model, device, loader, optimizer, task_type, use_aux_loss=False, verbose=True):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration", disable=not verbose)):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            # print('loss',loss,'dqn_loss',dqn_loss,'tol_loss',tol_loss)
...
def main(args, cluster=None):
    ...
    optimizer = torch.optim.AdamW([{'params': model.parameters(), 'lr': args.lr}, {'params': model.dqn.q_eval.parameters(), 'lr': 0.0005}], lr=args.lr, weight_decay=args.weight_decay)
    ...
    reward_curve = []
    save_every = 50
    save_path = 'ogb-mol/' +args.dataset + '/replay_1opt/' 
