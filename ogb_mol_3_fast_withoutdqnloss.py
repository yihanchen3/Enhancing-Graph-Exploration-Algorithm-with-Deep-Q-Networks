...
from model_3_single import AgentNet, add_model_args
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration", disable=not verbose)):
        ...
        with torch.autograd.set_detect_anomaly(True):
            if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
                ...
                print('dqn','reward',dqn.total_reward, 'e',dqn.epsilon,'loss_dqn',dqn.mean_loss,'agent_loss',loss.item())
...
def main(args, cluster=None):
    ...
    reward_curve = []
    save_every = 50
    save_path = 'ogb-mol/' +args.dataset + '/single/' 
