...
from model_3_replay import AgentNet, add_model_args
...
def train(model, device, loader, optimizer, optimizer_q ,task_type, use_aux_loss=False, verbose=True):
    model.train()
...
def main(args, cluster=None):
    ...
    print('HostName: {}, LoginName: {}, Current GPU ID: {}/{}, CodeFile: {}, StartTime: {}'.format(
        socket.gethostname(), os.getlogin(), os.environ['CUDA_VISIBLE_DEVICES'], torch.cuda.device_count(),__file__, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())), flush=True)
    ...
    optimizer_q = torch.optim.AdamW(model.dqn.q_eval.parameters(), lr=0.0001, weight_decay=args.weight_decay)
    scheduler = get_cosine_schedule_with_warmup(optimizer, args.warmup, args.epochs, min_lr_mult=args.min_lr_mult)
    scheduler_q = get_cosine_schedule_with_warmup(optimizer_q, args.warmup, args.epochs, min_lr_mult=args.min_lr_mult)
    ...
    reward_curve = []
    save_every = 50
    save_path = 'ogb-mol/' +args.dataset + '/0903/' 
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        loss, dqn = train(model, device, train_loader, optimizer,optimizer_q, dataset.task_type, use_aux_loss=args.use_aux_loss)
        scheduler.step()
        scheduler_q.step()
        ...
        print('Train:', train_perf, train_reward.item())
