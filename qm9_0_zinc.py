...
if args.complete_graph:
    ...
    path = osp.join(osp.dirname(osp.realpath(__file__)), 'data', 'ZINC_Subset')
    transform = T.Compose([MyTransform(), Complete(), T.Distance(norm=False)])
    dataset = ZINC(path, subset = True, transform=transform)
else:
    path = osp.join(osp.dirname(osp.realpath(__file__)), 'data', 'ZINC_Subset')
    dataset = ZINC(path, subset = True, transform=T.Compose([MyTransform(), T.Distance()]))
