...
test_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=20)
val_loader = DataLoader(val_dataset, batch_size=args.batch_size, num_workers=20)
train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=20)

print('deivce_0', dataset.data.edge_index.device)
print('deivce', device)

use_aux_loss = args.aux_loss
model = torch.nn.DataParallel(AgentNet(num_features=dataset.num_features, hidden_units=args.hidden_units, num_out_classes=1, dropout=args.dropout, num_steps=args.num_steps,
                ...
                final_readout_only=args.final_readout_only, num_edge_features=5, regression=True, qm9=True), device_ids=[0,1,2,3,4,5,6,7,8])
