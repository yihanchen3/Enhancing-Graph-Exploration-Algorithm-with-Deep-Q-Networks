...
from model_4 import AgentNet, add_model_args
...
# parser.add_argument('--num_worders', default=15)
...
def train(epoch):
    ...
    for data in train_loader:
        # with tqdm(total = len(train_loader), desc=f'Epoch {epoch}', unit='it') as pbar:
...
plot_learning_curve(np.arange(1, args.epochs+1), losses, 'Learning Curve', 'losses', save_path+args.dataset+'_3_losses.png')
plot_learning_curve(np.arange(1, args.epochs+1), val_errors, 'Learning Curve', 'val_errors', save_path+args.dataset+'_3_val_errors.png')
plot_learning_curve(np.arange(1, args.epochs+1), rewards, 'Learning Curve', 'rewards', save_path+args.dataset+'_3_rewards.png')